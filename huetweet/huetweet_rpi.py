...
class HueController(object):
    ...
    def data(self, envelope):
        ...
        tweet = str(envelope['params']['screen_name']) + str(envelope['params']['text']
        r, g, b = get_rgb_from_hex(hexstr)
        print(hexstr)
        print(tweet)

        self.set_arduino_color(hexstr)
    ...
    def set_arduino_color(self, hexstr):
        command = 'set_colors {}'.format(hexstr)
        self.logger.debug('sending arduino command={}'.format(command))
        neopix_hex.colorChange(hexstr)
	print(command)
        self.neopix_hex.colorChange.send(hexstr)
