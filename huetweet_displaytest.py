...
class HueController(object):
    ...
    def data(self, envelope):
        ...
        tweet = envelope['params']['tweet']['text']
        #r, g, b = get_rgb_from_hex(hexstr)
        print(hexstr)
        print(tweet)

        self.set_display_color(hexstr,tweet)
    ...
    def loop(self):
        ...
        if self.client in r:
            ...
            HueTweetDisplay.HueTweetDisplay(tweet,hexstr,False)
    ...
    def set_display_color(self, hexstr,tweet):
        command = 'set_colors {}'.format(hexstr)
        self.logger.debug('sending arduino command={}'.format(command))
        HueTweetDisplay.HueTweetDisplay(tweet,hexstr,True)
